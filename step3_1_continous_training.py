...
if __name__ == '__main__':
    ...
    previous_check_point_name = 'latest_checkpoint_cont_for_sasha.tar'
    
    model_path = './models/'
    model_name = 'Mesh_Segementation_MeshSegNet_50_classes_1500samples_for_sasha' #remember to include the project title (e.g., ALV)
    checkpoint_name = 'latest_checkpoint_cont_for sasha.tar'
    ...
    opt = optim.Adam(model.parameters(), amsgrad=True, lr=0.00005)
    ...
    best_val_dsc = max(val_mdsc, default=0)
    ...
    for epoch in range(epoch_init, num_epochs):
        ...
        if best_val_dsc < val_mdsc[-1]:
            ...
            torch.save({'epoch': epoch+1,
                        ...
                        'val_msen': val_msen,
                        'val_mppv': val_mppv},
                        os.path.join(model_path, '{}_new_best.tar'.format(model_name)))
            # save the checkpoint to drive(for colab training)
            if use_colab_drive:
        ...
        torch.save({'epoch': epoch + 1,
                    ...
                    'val_msen': val_msen,
                    'val_mppv': val_mppv},
                   os.path.join(model_path, '{}_new_las.tar'.format(model_name)))
        ...
        stat.to_csv('losses_metrics_vs_epoch_for_sasha.csv')
        if use_colab_drive:
